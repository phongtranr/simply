...
- The links point to `https://<your-domain>/archive/#year-YYYY-INDEX`, where `INDEX` matches the first post for that year, so make sure the archive route is enabled in `routes.yaml`.
- Ghost’s Content API returns at most 100 posts per request, so the strip reflects the most recent 100 entries; extend it with client-side pagination if you need deeper archives.
...
> Tip: Each year block now exposes an anchor such as `#year-2024-0`, which keeps the homepage archive strip in sync with the archive template.
